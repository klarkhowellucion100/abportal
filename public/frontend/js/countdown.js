  ...
  if (event.target.readyState === "complete") {
    ...
    const countdownFunction = setInterval(() => {
      ...
      for (let i = 0; i < countdownData.length; i++) {
        ...
        countdownData[i].hours = Math.floor(
          (distance % (1000 * 60 * 60 * 24)) / (1000 * 60 * 60)
        );
        countdownData[i].minutes = Math.floor(
        ...
        if (distance < 0) {
          ...
          countdownData[i].el.querySelector(".days").innerHTML =
            countdownData[i].days;
          countdownData[i].el.querySelector(".hours").innerHTML =
            countdownData[i].hours;
          countdownData[i].el.querySelector(".minutes").innerHTML =
            countdownData[i].minutes;
          countdownData[i].el.querySelector(".seconds").innerHTML =
            countdownData[i].seconds;
        }
      }
